      ...
      <div className="ct-main">
        <div className="ct-card">
          <div className="ct-empty">
            ...
            <Link className="ct-btn" to={`/compare${type ? `?type=${type}` : ''}`}>
    ...
    <div className="ct-main">
      ...
      <section className="ct-card">
        <div className="ct-card-head">
          <h1 className="ct-title-h1">{names}</h1>
        </div>
          ...
          <div className="ct-compare-head">
            {products.map((product, index) => (
              <React.Fragment key={product._id}>
                {index > 0 && <div className="ct-compare-head-vs">VS</div>}
                <div className="ct-compare-head-item">
                  ...
                  <Link className="ct-compare-head-name" to={cheminProduit(type, product)}>
                    {product.name}
                  </Link>
                  <span className="ct-compare-head-brand">{product.brand}</span>
                </div>
              </React.Fragment>
            ))}
          </div>
      ...
      <nav className="ct-anchor-nav ct-anchor-nav-sticky" aria-label="Sections du comparatif">
      ...
      <div id="radar" ref={register('radar')}>
        {p1 && p2 && (
          <section className="ct-card">
            <div className="ct-card-head">
              <h2 className="ct-title-h2">Profil comparé</h2>
      ...
      <div id="specs" ref={register('specs')}>
        {/* Hors de la carte, pour la même raison que la barre d'ancres :
            `overflow: hidden` sur .ct-card annule `position: sticky`. Le
            conteneur neutre limite la barre à la zone du tableau — elle
            n'apparaît pas ailleurs sur la page. */}
        <div className="ct-sticky-names">
          <span className="ct-sticky-names-label">Comparés :</span>
          {products.map((product, index) => (
            <React.Fragment key={product._id}>
              {index > 0 && <span className="ct-sticky-names-vs">vs</span>}
              <span className="ct-sticky-names-item">{product.name}</span>
            </React.Fragment>
          ))}
        </div>
